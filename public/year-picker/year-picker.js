(function($) {
	var
		...
		picker_render_calendar = function() {
			...
			//after
			if(w.find('li.pick-n').length < 0) {
				var
					e = 0 - w.find('li.pick-n').length;
				for(var i = 0 ; i <= e; i) {
				...
				if(pickers[picker.id].lock==='from') {
					if(get_current('y')<=get_today('y')) {
						if(get_current('m')==get_today('m')) {
							get_picker_els('.pick-lg .pick-lg-b li.pick-v[data-value="'+get_today(0)+'"]')
							.prevAll('li')
							.addClass('pick-lk')
						}
						else {
		...
		picker_fills = function() {
			...
			pickers[picker.id].key['d'].max =  [][m-1];
	...
	var
		picker_node_el = 'div.yearpicker.picker-focus';
	...
	$.fn.yearPicker = function(options) {
		return $(this).each(function(){
			if($(this).is('input')&&!$(this).hasClass('picker-input')) {
				...
				var
					input = $(this),
					id = 'yearpicker-' + Object.keys(pickers).length;
				...
				var
					...
					picker_format = input.data('format') || 'Y',
					...
					picker_min_year = (input.data('min-year')&&is_int(input.data('min-year'))) ? input.data('min-year') : new Date().getFullYear()-2,
				...
				$('<div>', {
					class: 'yearpicker ' + picker_modal + ' ' + picker_theme + ' ' + picker_fx_class + ' ' + picker_large_class,
					id: id,
					html: $('<div>', {
						class: 'picker'
					})
